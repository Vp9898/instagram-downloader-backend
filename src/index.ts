...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // --- نقطة النهاية الجديدة للوكيل ---
    if (url.pathname === '/image-proxy') {
      const imageUrl = url.searchParams.get('url');
      if (!imageUrl) {
        return new Response('Missing image URL', { status: 400 });
      }
      // قم بجلب الصورة وإعادتها مباشرة
    ...
    try {
      if (!env.INSTA_API_KEY) {
        return new Response(JSON.stringify({ error: 'SECRET_NOT_FOUND_IN_ENV' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' }});
      }
